...
def find_notebooks(backup_dir: Path) -> List[Dict]:
    ...
    if not files_dir.exists():
        logging.error("Backup files directory not found: %s", files_dir)
        return []
...
def convert_v6_file_with_rmc(rm_file: Path, output_file: Path) -> bool:
    ...
    try:
        ...
        logging.debug("rmc SVG failed for %s: %s", rm_file.name, result.stderr)
        return False
...
def convert_v5_file_with_rmrl(rm_file: Path, output_file: Path) -> bool:
    ...
    try:
        ...
        try:
            svg_data = rmrl.render(str(rm_file))  # Remove template parameter
            if svg_data:
...
def convert_notebook(notebook: Dict, output_dir: Path, backup_dir: Path) -> Dict:  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,unused-argument
...
def main(backup_dir: Path, output_dir: Optional[Path], verbose: bool, sample: Optional[int], updated_only: Optional[Path]):  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
